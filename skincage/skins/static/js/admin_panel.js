...
function abrirModalSkin(skin = null) {
  const modal = document.getElementById("modal-skin");
  const accion = document.getElementById("modal-skin-accion");
  ...
  if (skin) {
    accion.textContent = "Editar Skin";
    document.getElementById("skin-id-editar").value = skin.id;
    document.getElementById("skin-nombre").value = skin.nombre;
    document.getElementById("skin-precio").value = skin.precio;
    document.getElementById("skin-desgaste").value = skin.desgaste;
    document.getElementById("skin-stock").value = skin.stock;
    document.getElementById("skin-categoria").value = skin.categoria;
    document.getElementById("skin-rareza").value = skin.rareza;
    document.getElementById("skin-stattrack").checked = skin.stattrack;
  } else {
    accion.textContent = "Nueva Skin";
    document.getElementById("skin-id-editar").value = "";
    document.getElementById("form-skin").reset();
  }

  modal.classList.add("visible");
  document.getElementById("skin-nombre").focus();
}

function cerrarModalSkin() {
  document.getElementById("modal-skin").classList.remove("visible");
}

function abrirModalEliminar(id, nombre) {
  skinIdEliminar = id;
  document.getElementById("skin-nombre-eliminar").textContent = `"${nombre}"`;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document
    .getElementById("btn-form-cancelar")
    .addEventListener("click", cerrarModalSkin);
  document
    .getElementById("modal-eliminar-cerrar")
...
async function guardarSkin() {
  const errorDiv = document.getElementById("form-skin-error");
  const btnGuardar = document.getElementById("btn-form-guardar");
  const idEditar = document.getElementById("skin-id-editar").value;

  const payload = {
    nombre: document.getElementById("skin-nombre").value.trim(),
    precio: parseFloat(document.getElementById("skin-precio").value),
    desgaste: parseFloat(document.getElementById("skin-desgaste").value),
    stock: parseInt(document.getElementById("skin-stock").value),
    categoria: document.getElementById("skin-categoria").value,
    rareza: document.getElementById("skin-rareza").value,
  ...
  try {
    ...
    const url = esEdicion ? `/api/skin-crud/${idEditar}/` : "/api/skin-crud/";
    ...
    if (!respuesta.ok) {
      ...
      const mensajeError =
        Object.values(datos).flat().join(" ") || "Error al guardar la skin.";
    ...
    errorDiv.textContent = fallo.message;
    errorDiv.style.display = "block";
  } finally {
...
async function eliminarSkin() {
  if (!skinIdEliminar) return;
  ...
  try {
    const respuesta = await fetch(`/api/skin-crud/${skinIdEliminar}/`, {
      method: "DELETE",
      headers: { "X-CSRFToken": getCsrfToken() },
    });

    if (!respuesta.ok) throw new Error("No se pudo eliminar la skin.");
...
async function fetchUsuarios() {
  ...
  try {
    ...
    usuarios.forEach((usuario) => {
      ...
      fila.className = "fila-flex";

      const nombreSpan = document.createElement("span");
      nombreSpan.className = "titulo-entrada";
      nombreSpan.textContent = usuario.username;
      fila.appendChild(nombreSpan);

      const etiquetaRol = document.createElement("span");
      etiquetaRol.className = usuario.is_staff ? "etiqueta admin" : "etiqueta";
      ...
      const saldoTexto =
        usuario.balance !== undefined ? ` • Balance: ${usuario.balance}€` : "";
...
async function fetchReservas() {
  ...
  try {
    ...
    lista.forEach((reserva) => {
      ...
      tituloSpan.className = "titulo-entrada";
      ...
      etiquetaEstado.className = `etiq-estado badge-${reserva.estado}`;
      const textoEstado =
...
function pintarPaginaSkins() {
  ...
  skinsPagina.forEach((skin) => {
    ...
    btnEditar.className = "btn-control btn-mod";
    btnEditar.title = "Editar skin";
    btnEditar.innerHTML = '<i class="fa-solid fa-pen-to-square"></i>';
    btnEditar.addEventListener("click", () => abrirModalSkin(skin));
    acciones.appendChild(btnEditar);

    const btnEliminar = document.createElement("button");
    btnEliminar.className = "btn-control btn-del";
    btnEliminar.title = "Eliminar skin";
    btnEliminar.innerHTML = '<i class="fa-solid fa-trash"></i>';
    btnEliminar.addEventListener("click", () =>
      abrirModalEliminar(skin.id, skin.nombre),
    );
    ...
    subtituloSpan.style.flexWrap = "wrap";
    ...
    desgasteSpan.textContent = skin.desgaste
      ? `Float: ${skin.desgaste}`
      : "N/A";
